...
--Procedmiento almacenado para DATOS DE LA VENTA
CREATE PROC sp_Venta
@CI VARCHAR(11),
@CODIGO VARCHAR (10),
@CANTIDAD_VENDIDA VARCHAR (50),
@PRECIO_UNITARIO MONEY,
@PRECIO_TOTAL MONEY
AS
BEGIN
	DECLARE @CANTIDAD INT
	INSERT INTO VENTAS VALUES(@CI, @CODIGO, GETDATE(),@CANTIDAD_VENDIDA,
	@PRECIO_UNITARIO,@PRECIO_TOTAL)

	SELECT @CANTIDAD=CANTIDAD FROM PRODUCTO WHERE CODIGO=@CODIGO
	SET @CANTIDAD=@CANTIDAD-@CANTIDAD_VENDIDA

	UPDATE PRODUCTO SET CANTIDAD=@CANTIDAD WHERE CODIGO=@CODIGO
END

--Procedmiento almacenado para BUSQUEDA DE CLIENTE
CREATE PROC sp_BusquedaCliente
@CI VARCHAR (10)
AS
BEGIN
...
--Procedmiento almacenado para BUSQUEDA DE PRODUCTO
CREATE PROC sp_BusquedaProducto
@CODIGO VARCHAR (10)
AS
BEGIN
